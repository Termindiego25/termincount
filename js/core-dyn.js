...
function generatePoll(){
	totalVotes = 0; numOptions = 0; stackKey = [];
	document.querySelectorAll('.progress-bar').forEach(pb=>{ pb.setAttribute('aria-valuenow','0'); pb.style.width='0%'; });
	...
	var slots = inputs.filter(i=> i.value.trim() !== '').map((i,idx)=> ({ slotlabel: i.value.trim(), barType: colors[(idx+1)%4] }));
	if(slots.length === 0){
		buildOptions();
	} else {
		pollData = { question: question || (window.I18N?I18N.t('poll.default.question'):'Votación'), slots: slots };
...
function generateTopList( options ){
	...
	jQuery.each(options.slots.slice(0,4), function (i, slot) { // asegurar máximo 4
		var txt = (window.I18N ? I18N.t('shortcut.template',{n:(i+1), label:slot.slotlabel}) : "Tecla '" + (i+1) + "' o click en el contador: " + slot.slotlabel);
...
function buildOptions(options) {
	var defaults = {
		...
		slots: [
			{ slotlabel: window.I18N?I18N.t('poll.default.opt.afavor'):'A favor', barType:'success'},
			{ slotlabel: window.I18N?I18N.t('poll.default.opt.encontra'):'En contra', barType:'danger'},
			{ slotlabel: window.I18N?I18N.t('poll.default.opt.blanco'):'Blanco', barType:'warning'},
			{ slotlabel: window.I18N?I18N.t('poll.default.opt.nulo'):'Nulo', barType:'info'}
		]
	};
	if(!options){ options = defaults; generateTopList(options); }
