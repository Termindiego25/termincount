<script lang="ts">
	...
	function startPoll() {
		const customPoll = createCustomPoll(title, optionInputs, currentLang);
		poll = customPoll.slots.length === 0 ? createDefaultPoll(currentLang) : customPoll;
		counts = Array.from({ length: poll.slots.length }, () => 0);
		history = [];
		isRunning = true;
	}

	function vote(index: number) {
		if (!isRunning || !poll || index < 0 || index >= poll.slots.length) return;
		counts = counts.map((count, countIndex) => (countIndex === index ? count + 1 : count));
	...
	function undoLastVote() {
		...
		counts = counts.map((count, countIndex) =>
	...
	function handleGlobalKeydown(event: KeyboardEvent) {
		...
		if (Number.isInteger(numeric) && poll && numeric >= 1 && numeric <= poll.slots.length) {
			event.preventDefault();
			vote(numeric - 1);
		}
	}
	...
	onMount(() => {
		currentLang = detectLanguage(navigator.language, readStorage(languageStorageKey));
	...
	<div class="container app-container py-4 flex-grow-1">
		...
		{#if isRunning && poll}
			<nav aria-label="Atajos de teclado" class="mb-2">
				<ul class="list-inline small mb-0" id="lista">
					{#each poll.slots as slot, index}
						<li class="list-inline-item me-3">
							{t(currentLang, 'shortcut.template', { n: index + 1, label: slot.slotlabel })}
						</li>
					{/each}
			...
			<h2 id="question" class="text-primary h4">{poll.question}</h2>
		{/if}
			...
			{#if !isRunning}
				<div class="col-12" id="input">
					...
					<div class="d-flex flex-wrap gap-2 mb-2">
						...
						<button class="btn btn-primary" type="button" id="startPoll" aria-describedby="help-default" onclick={startPoll}>
							{t(currentLang, 'btn.start')}
						</button>
					</div>
				</div>
			{:else if poll}
							...
							<li class="mb-3">
								...
								>
									<span class="option-heading h6 mb-2 d-flex align-items-center gap-2 flex-wrap">
										...
										<span id={`slot-${index}-label`} class="badge text-bg-secondary">{count}</span>
										<span class="option-percentage text-body-secondary small">{pct}%</span>
									</span>
									<span class="progress vote-progress">
										<progress
											id={`slot-${index}`}
											class={`vote-meter ${slot.barType}`}
											value={pct}
											max="100"
											aria-valuetext={`${count} (${pct}%)`}
										>
											{pct}%
										</progress>
									</span>
